...
export function startBot() {
	...
	bot.onText(/\/infos/, async (msg) => {
		...
		const lastArticle: StoredArticle | undefined = recentArticles[0];
...
└ Stockage : SQLite (volume persistant)
	...
	bot.onText(/\/ask (.+)/, async (msg, match) => {
		...
		const localArticles = searchArticles(question);
		let context = "";

		if (localArticles.length > 0) {
			context += "📚 Articles de ta veille :\n";
			context += localArticles.map((a) => `- ${a.title} (${a.source})\n  ${a.summary}`).join("\n");
		}
...
Si le contexte est insuffisant, dis-le clairement mais reste focalisé sur 2026.`,
	...
	bot.onText(/\/analyze/, async (msg) => {
		...
		const context = articles
			.slice(0, 50)
			.map((a) => `- [${a.source}] ${a.title} : ${a.summary}`)
			.join("\n");
...
export async function notifyNewArticles(articles: EnrichedArticle[]) {
	const bySource = articles.reduce<Record<string, EnrichedArticle[]>>((acc, a) => {
	...
	for (const [source, sourceArticles] of Object.entries(bySource)) {
		...
		const message = `🆕 *${source}* — ${sourceArticles.length} nouveaux articles\n\n${lines}`;
